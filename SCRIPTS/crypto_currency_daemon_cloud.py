  ...
  for ticker_value_in_array in ticker_values_in_array:
      try:
        if (ticker_value_in_array[0].split(":")[1] in currencies):
          api_command_str = "python fbbs_api_cloud.py '" + str(ticker_value_in_array[0]) + ":" + str(update_rate_mins) + "min'"
          ...
          api_command_str = "python fbbs_api_cloud.py '" + str(ticker_value_in_array[0]) + ":" + str(update_rate_mins) + "min' " + str(ticker_value)
          print(api_command_str)
          fbbs_api_obj = subprocess.check_output(api_command_str, shell=True)
          updated_values += 1
      except:
        pass
  api_command_str = "python fbbs_api_cloud.py 'Ticker:CryptoUpdates" + ":" + str(update_rate_mins) + "min'"
  print(api_command_str)
  fbbs_api_obj = subprocess.check_output(api_command_str, shell=True)
  api_command_str = "python fbbs_api_cloud.py 'Ticker:CryptoUpdates" + ":" + str(update_rate_mins) + "min' " + str(updated_values)
  print(api_command_str)
  fbbs_api_obj = subprocess.check_output(api_command_str, shell=True)
