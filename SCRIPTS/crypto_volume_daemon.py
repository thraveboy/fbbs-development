  ...
  for coin in coin_values:
    try:
      ...
      if coin_name in currencies:
        api_command_str = "python fbbs_api.py 'TradeVolume" + ":" + coin_name + ":" + str(update_rate_mins) + "min'"
        print(api_command_str)
        fbbs_api_obj = subprocess.check_output(api_command_str, shell=True)
        api_command_str = "python fbbs_api.py 'TradeVolume" + ":" + coin_name + ":" + str(update_rate_mins) + "min" + " " + str(coin['Volume_24h']) + "'"
        print(api_command_str)
        fbbs_api_obj = subprocess.check_output(api_command_str, shell=True)
    except:
      pass
