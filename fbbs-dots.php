...
function curvePointValue(index) {
  ...
  var current_point = { x: 0.5, y: 0.5, r:0.5, label: index};
            ...
            animation: {
                onComplete: function () {
                    ...
                    for (var j = 0; j < data_array.length; j++) {
                         ...
                         var pointLoc = {
                             x: chart_x_max*clean_y_div,
                             y: chart_y_max-(chart_y_max*clean_x_div)
                           };

                         ctx.fillText(current_data.label, pointLoc.x,
                                      pointLoc.y);
                         ctx.beginPath();
                         ctx.arc(pointLoc.x, pointLoc.y,
                                 curvePoint.r * ctx.canvas.width, 0,
                                 2*Math.PI);
                         ctx.fillStyle = getDataColor(current_data);
                         ctx.fill();
                      };
                  }
             },
